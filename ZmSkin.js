 ...
 * ***** END LICENSE BLOCK *****
 */
// Declara listas de URL's que devem acessar as configurações

const urlsPermitidas = ["https://zimbra.com/","https://zimbra.51ct.eb.mil.br/"];
// Valida a URL de Acesso do cliente
const urlAtual = window.location.href;
const colorText = "#1d1d1f;"
try {
    //Pega a DIV do login e salva em uma contante
    const TelaLoginPenso = document.getElementById("modifiedLogin");
    //Alterando o LABEL do "Bem Vindo"
    TelaLoginPenso.getElementsByClassName("signIn")[0].style.textAlign = "center";
    TelaLoginPenso.getElementsByClassName("signIn")[0].style.color = colorText;
    //TelaLoginPenso.getElementsByClassName("signIn")[0].innerText="Webmail Funcional"+String.fromCharCode(13)+"51º Centro de Telemática";
    TelaLoginPenso.getElementsByClassName("signIn")[0].innerText="Correio Eletrônico Funcional";
    //Alterando os INPUTS
    TelaLoginPenso.getElementsByTagName("input")[2].placeholder = "Usuário";
    TelaLoginPenso.getElementsByTagName("input")[3].placeholder = "Senha";
    //Alterando os Labels
    TelaLoginPenso.getElementsByTagName("label")[0].style.visibility = "hidden";
    TelaLoginPenso.getElementsByTagName("label")[1].style.visibility = "hidden";
    TelaLoginPenso.getElementsByTagName("label")[2].style.color = colorText;
    TelaLoginPenso.getElementsByTagName("label")[3].style.color  = colorText;
} catch(e){}
try {
    function verificarURLRodape() {
     //Alterando o Copyright
     document.getElementById("ZLoginNotice").style.color = colorText;
     document.getElementById("ZLoginNotice").style.position = "relative";
     document.getElementById("ZLoginNotice").style.boxSizing = "border-box";
     document.getElementById('ZLoginNotice').innerHTML = '';
    };
    verificarURLRodape();
} catch(e){}
/* try {
    const ValidaTelaLoginPenso = document.getElementById("modifiedLogin");
    //Criando nova Div de Divulgação
    const TelaLoginPensoFundoMarketing = document.createElement("div");
    TelaLoginPensoFundoMarketing.style.width = "50%";
    TelaLoginPensoFundoMarketing.style.height = "100vh";
    TelaLoginPensoFundoMarketing.style.background = "rgba(0,168,107,255)";
    TelaLoginPensoFundoMarketing.style.marginLeft = "50%";
    TelaLoginPensoFundoMarketing.style.zIndex = "2";
    TelaLoginPensoFundoMarketing.style.boxSizing = "border-box";
    if (window.getComputedStyle(ValidaTelaLoginPenso).display === 'table') {
        document.body.appendChild(TelaLoginPensoFundoMarketing);
    }
    function verificarURLMKT() {
        for (var i=0;i<urlsPermitidas.length;i++){
            if (urlAtual.includes(urlsPermitidas[i])){
                //Cria nova div de contenedor para a Div de Divulgação
                const TelaLoginPensoFundoMarketingConteudo = document.createElement("div");
                TelaLoginPensoFundoMarketing.appendChild(TelaLoginPensoFundoMarketingConteudo);
                TelaLoginPensoFundoMarketingConteudo.style.position = "relative";
                TelaLoginPensoFundoMarketingConteudo.style.margin = "auto";
                TelaLoginPensoFundoMarketingConteudo.style.width = "100%";
                TelaLoginPensoFundoMarketingConteudo.style.height = "100%";
                TelaLoginPensoFundoMarketingConteudo.style.top = "0%";
                TelaLoginPensoFundoMarketingConteudo.style.fontFamily = 'Roboto,"Helvetica Neue",Helvetica,Arial,"Liberation Sans",sans-serif';
                TelaLoginPensoFundoMarketingConteudo.style.textAlign = "center";
                //TelaLoginPensoFundoMarketingConteudo.style.transform = "translateY(-50%)";
                TelaLoginPensoFundoMarketingConteudo.innerHTML = '<img src="https://intranet.51ct.eb.mil.br/ebmail/51CT.jpg" width="100%" height="100%" style="object-fit: contain;"/>';
            };
        };
    };
    verificarURLMKT();
...
*/
//Remove o botão de Videoconferência do ImMail
try{
    function monitorarAlteracoesDeURL() {
    ...
    function urlMudou() {
        try {
            var BotaoConfImMNameOne = "zb__APPT-";
            var BotaoConfImMNameTwo = "__IMMAILVCBUTTON";
            for (var IncreBTNIdName = 0; IncreBTNIdName < 100; IncreBTNIdName++) {
                var BotaoConfImMName = BotaoConfImMNameOne + IncreBTNIdName + BotaoConfImMNameTwo;
                var BotaoConfIMM = document.querySelector("#" + BotaoConfImMName);
                if (BotaoConfIMM) {
                    BotaoConfIMM.style.display = "none";
                }
            }
        } catch (e) {}
        try {
            var BotaoConfIMMHome = document.querySelector("#" + "zb__CLV-main__IMMAILVCEXTBUTTON");
            if (BotaoConfIMMHome) {
                BotaoConfIMMHome.style.display = "none";
            }
        } catch (e) {}
        try {
        var ModerSelector = document.getElementById("prefs.Pages#General_PASSWORD_LABEL");
            ModerSelector.nextElementSibling.style.display = 'none';
        } catch (e) {}
    }
    setTimeout( function() {
        try {
    ...
    monitorarAlteracoesDeURL();
} catch(e){}
//Antigo codigo da Skin
...
//Altera Id's de itens na skin
setTimeout(function() {
    var ListaTodosElementos = document.getElementsByClassName("ZWidgetTitle");
    var ValidadorRecurso = '';
    for (var i = 0; i < ListaTodosElementos.length; i++) {
        if (ListaTodosElementos[i].innerText === "Porta-arquivos") {
            ListaTodosElementos[i].id = 'EmailZimbraPortaArquivos';
            ValidadorRecurso = 'AccountCollab';
        } else if (ListaTodosElementos[i].innerText === "Penso Meet") {
            ListaTodosElementos[i].id = 'EmailZimbraMeeting';
        } else if (ListaTodosElementos[i].innerText === "Penso Chat") {
            ListaTodosElementos[i].id = 'EmailZimbraChatting';
        }
    }
    if (ValidadorRecurso === '') {
        ValidadorRecurso = 'AccountBasic';
    }
    //console.log(ValidadorRecurso);
    //window.userGuiding.identify(ValidadorRecurso);
}, 5000);
//Cria o alerta do chat e altera os links dos itens de ajuda
setTimeout(function() {
    //Altera o Funcionamento ao clicar no logo
    try {
        var LoginLink = document.getElementById('skin_spacing_logo').getElementsByTagName('a')[0];
        LoginLink.setAttribute('href','#');
        LoginLink.setAttribute('target','');
        const primeiroElemento = document.querySelector('#skin_spacing_logo');
        const segundoElemento = document.querySelector('#zb__App__Mail');
        if (primeiroElemento) {
            primeiroElemento.onclick = function () {
                segundoElemento.click()
            };
        };
    } catch(e) {}
    //Adiciona alerta de nova mensagem no chat
    try {    
        var ListaTabs = document.getElementsByClassName("ZWidgetTitle");
        var target = '';
        for (var i = 0; i <= ListaTabs.length;i++){
            if(ListaTabs[i].innerText.split(" ")[1] === "Chat"){
                target = ListaTabs[i];
                break;
            };
        };
        if (target.innerText.length <= 10) {
            target.parentElement.parentElement.parentElement.parentElement.style.background = "";
        } else {
            target.parentElement.parentElement.parentElement.parentElement.style.background = "rgba(255,200,0,1)";
        };
        var observer = new MutationObserver(function (mutations) {
            mutations.forEach(function (mutation) {
                if (mutation.type === "childList" && target.innerText.length > 10 ) {
                    target.parentElement.parentElement.parentElement.parentElement.style.background = "rgba(255,200,0,1)";
                } else {
                    target.parentElement.parentElement.parentElement.parentElement.style.background = "";
                };
    ...
    //Altera os links das bases de ajuda
    try {
        document.getElementById("modernClientLink").style.display = 'none';
        document.getElementById("modernClientLink").parentElement.parentElement.nextElementSibling.style.display = 'none';
        var HelpBtnParent = document.getElementById("onlinehelp").parentElement;
        document.getElementById("onlinehelp").parentElement.innerHTML = '<a href="https://penso.movidesk.com/kb" target="_blank"><div class="ZMenuItem ZWidget ZHasText" role="menuitem" tabindex="0" parentid="POPUP_DWT30" aria-label="Central de ajuda on-line" style="position: static; overflow: visible;"><table role="none" class="ZWidgetTable ZMenuItemTable ZMenuItemBorder"><tbody><tr id="POPUP_onlinehelp"><td id="onlinehelp_check" class="ZCheckIcon"><div></div></td><td id="onlinehelp_left_icon" class="ZLeftIcon ZWidgetIcon"></td><td id="onlinehelp_title" class="ZWidgetTitle">Central de ajuda on-line</td><td id="onlinehelp_dropdown" class="ZDropDown"><div></div></td></tr></tbody></table></div></a>';
        HelpBtnParent.onmouseover = function(){
            HelpBtnParent.style.background = "rgba(0,0,0,0.1)"
        };
        HelpBtnParent.onmouseout = function(){
            HelpBtnParent.style.background = "";
        };
        var NovasFuncoesBtnParent = document.getElementById("newFeatures").parentElement;
        document.getElementById("newFeatures").parentElement.innerHTML = '<a href="https://penso.movidesk.com/kb/article/423082/novidades-chegando-tudo-o-que-ha-de-novo-no-zimbra-10?ticketId=&q=Novidades" target="_blank"><div class="ZMenuItem ZWidget ZHasText" role="menuitem" tabindex="0" parentid="POPUP_DWT30" aria-label="Novos recursos" style="position: static; overflow: visible;"><table role="none" class="ZWidgetTable ZMenuItemTable ZMenuItemBorder"><tbody><tr id="POPUP_newFeatures"><td id="newFeatures_check" class="ZCheckIcon"><div></div></td><td id="newFeatures_left_icon" class="ZLeftIcon ZWidgetIcon"></td><td id="newFeatures_title" class="ZWidgetTitle">Novos recursos</td><td id="newFeatures_dropdown" class="ZDropDown"><div></div></td></tr></tbody></table></div></a>';
        NovasFuncoesBtnParent.onmouseover = function(){
            NovasFuncoesBtnParent.style.background = "rgba(0,0,0,0.1)"
        };
        NovasFuncoesBtnParent.onmouseout = function(){
            NovasFuncoesBtnParent.style.background = "";
        };
    } catch(e){}
},5000);
...
try {
    // Selecione a div existente pelo id
	var divModifiedLogin = document.getElementById("modifiedLogin");

	divModifiedLogin.className = "main-left";

    // Crie a nova div pai (main-container)
	var divMainContainer = document.createElement("div");
	divMainContainer.className = "main-container";

    // Insira a nova div pai no DOM antes da div existente
	divModifiedLogin.parentNode.insertBefore(divMainContainer, divModifiedLogin);

    // Mova a div existente (modifiedLogin) para dentro da nova div pai
	divMainContainer.appendChild(divModifiedLogin);

    // Crie a nova div que ficará após a div existente (main-right)
	var newDiv = document.createElement("div");
   	newDiv.className = "main-right"; // Substitua "main-right" pelo nome da classe que você deseja usar

    // Insira a nova div logo após a div existente (modifiedLogin)
    	divMainContainer.appendChild(newDiv); // Agora a div main-right será irmã de modifiedLogin dentro de main-container

	// REMOVE A EXIBIÇÃO DO TIPO DE LOGIN
	document.getElementsByClassName('versionBlock')[0].style.display='none';	

	// ALTERA A CLASSE DO BOTÃO DE LOGIN
	document.getElementById('loginButton').className = 'new-btn';

	var imgLogoElement = document.createElement('img');
	imgLogoElement.src='/skins/Esmeralda/img/ebmail-logo.png';
	imgLogoElement.className='img-logo';
	var divModernContentBox = document.querySelector('.modernContentBox');
	divModernContentBox.insertBefore(imgLogoElement, divModernContentBox.firstChild);

	var imgCitexElement = document.createElement('img');
	imgCitexElement.src='/skins/Esmeralda/img/citex.png';
	imgCitexElement.className='img-citex';
	var divMainRight = document.querySelector('.main-right');
	divMainRight.insertBefore(imgCitexElement, divMainRight.firstChild);
	var divCitexTexto = document.createElement('div');
	divCitexTexto.className = 'texto-citex';
	divCitexTexto.innerText= 'Conectando o Exército Brasileiro';
	divMainRight.appendChild(divCitexTexto, divMainRight)
